...
public class ConfigAddTransactionRestHandlerTest {
	
	@Spy
	@InjectMocks
	private ConfigAddTransactionRestHandler configAddTransactionRestHandler;
	...
	public void processTest() {
		...
		String s="{}";
		when(((ConfigRestHandler)configAddTransactionRestHandler).getCustomJsonProcessingUtil().createJsonString(any(),any())).thenReturn(s);
		configAddTransactionRestHandler.process(inputParamMap, inputDesign);
	}
	...
	public void processTestError() throws SalesBusinessException {
		...
		String s="{}";
		when(((ConfigRestHandler)configAddTransactionRestHandler).getCustomJsonProcessingUtil().createJsonString(any(),any())).thenReturn(s);
		doThrow(new SalesBusinessException("")).doNothing().when(((ConfigRestHandler)configAddTransactionRestHandler)).triggerRestClient(any(), any(), any(), any(), any());
		configAddTransactionRestHandler.process(inputParamMap, inputDesign);
	}
	...
	public void processTestError1() throws SalesBusinessException  {
		...
		String s="{}";
		when(((ConfigRestHandler)configAddTransactionRestHandler).getCustomJsonProcessingUtil().createJsonString(any(),any())).thenReturn(s);
		configAddTransactionRestHandler.process(null, inputDesign);
	}
	
	@Test
	public void initTest() {
		configAddTransactionRestHandler.init();
	}
