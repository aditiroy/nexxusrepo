...
public class ConfigInSystemRestHandlerTest {
	
	@Spy
	@InjectMocks
	private ConfigInSystemRestHandler configInSystemRestHandler;
	...
	public void processTest() {
		...
		String s="{}";
		when(((ConfigRestHandler)configInSystemRestHandler).getCustomJsonProcessingUtil().createJsonString(any(),any())).thenReturn(s);
		configInSystemRestHandler.process(inputParamMap, inputDesign);
	}
	...
	public void processTestError() throws SalesBusinessException {
		...
		String s="{}";
		when(((ConfigRestHandler)configInSystemRestHandler).getCustomJsonProcessingUtil().createJsonString(any(),any())).thenReturn(s);
		doThrow(new SalesBusinessException("")).doNothing().when(((ConfigRestHandler)configInSystemRestHandler)).triggerRestClient(any(), any(), any(), any(), any());
		configInSystemRestHandler.process(inputParamMap, inputDesign);
	}
	...
	public void processTestError1() throws SalesBusinessException  {
		...
		String s="{}";
		when(((ConfigRestHandler)configInSystemRestHandler).getCustomJsonProcessingUtil().createJsonString(any(),any())).thenReturn(s);
		configInSystemRestHandler.process(null, inputDesign);
	}
	
	@Test
	public void initTest() {
		configInSystemRestHandler.init();
	}
