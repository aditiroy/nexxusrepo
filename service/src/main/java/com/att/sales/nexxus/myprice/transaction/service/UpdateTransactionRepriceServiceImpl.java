...
@Service("updateTransactionRepriceServiceImpl")
public class UpdateTransactionRepriceServiceImpl extends BaseServiceImpl{
	...
	public ServiceResponse updateTransactionRepriceService(LinkedHashMap<String, Object> requestMap)
			throws SalesBusinessException {
		logger.info("Entering updateTransactionRepriceService() method");
		...
		try {
			...
			qualifyServiceResponse = httpRestClient.callHttpRestClient(env.getProperty("myprice.updateTransactionReprice")
					.replace("{transactionId}", myPriceTransId), HttpMethod.POST, null, emptyRequest, 
					headers, proxy);
		} catch (SalesBusinessException e) {
			logger.error("Exception occured while calling update transaction Reprice service {}", e);
		...
		logger.info("Exiting updateTransactionRepriceService() method");
		return response;
	}
}
