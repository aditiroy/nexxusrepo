...
@Component("aseod3PAMileageReqRatesServiceImpl")
public class Aseod3PAMileageReqRatesServiceImpl extends BaseServiceImpl implements Asenod3PAService<Aseod3PAMileageReqRatesResponse>{


	/** The Constant logger. */
	private static final Logger logger = LoggerFactory.getLogger(Aseod3PAMileageReqRatesServiceImpl.class);
	...
	@Override
	public Aseod3PAMileageReqRatesResponse process(String transactionId) throws SalesBusinessException {
		logger.info("Entering processMileageReqRates method {}", transactionId);
		Aseod3PAMileageReqRatesResponse response=null;
		try {
			//myPriceTransactionUtil.setSystemProperties();
			String uri = env.getProperty("myprice.mileageReqRatesArrContainer");
			...
			if (null != transResponse) {
				response = (Aseod3PAMileageReqRatesResponse) restClient.processResult(transResponse,
						Aseod3PAMileageReqRatesResponse.class);
