...
@Component("configInSystemRestHandler")
public class ConfigInSystemRestHandler extends ConfigRestHandler{
	
	private static final Logger log = LoggerFactory.getLogger(ConfigInSystemRestHandler.class);

	@PostConstruct
	protected void init() {
		setWsName(CustomJsonConstants.CONFIG_SYSTEM);
	}

	@Override
	public Map<String, Object> process(Map<String, Object> requestMap, String inputDesign){
		log.info("Inside process for ConfigInSystemRestHandler","");
		Map<String, Object> responseMap =new HashMap<String, Object>();
		responseMap.put(MyPriceConstants.RESPONSE_STATUS, false);
		try {
			if(StringUtils.isNotEmpty(inputDesign)) {
				requestMap.put(CustomJsonConstants.RULE_TYPE, CustomJsonConstants.CONFIG_SYSTEM);
