...
@Component
public class ConfigAndUpdatePricingUtilIgloo {
	
	private static final Logger log = LoggerFactory.getLogger(ConfigAndUpdatePricingUtilIgloo.class);
	...
	protected String customeCodeProcessing(NxMpConfigMapping mappingData,String inputDesignDetails,
		...
		if(StringUtils.isNotEmpty(mappingData.getVariableName())){
			if((MyPriceConstants.ETHERNET.equals(mappingData.getOffer()) || 
				...
				return requestMap.get(mappingData.getVariableName()).toString();
			}
		}
		return "";
	...
	protected String convertAccessSpeed(String input,String offerName) {
		if(MyPriceConstants.ETHERNET.equals(offerName)) {
			return converAccessSpeedEthernet(input);
		}else if(MyPriceConstants.TDM.equals(offerName)) {
			return converAccessSpeedTDM(input);
		}
		return null;
	}
	
	protected String converAccessSpeedEthernet(String input) {
	...
	protected String converAccessSpeedTDM(String input) {
		if(StringUtils.isNotEmpty(input)) {
